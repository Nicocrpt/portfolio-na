...
gsap.to('.mainTitle', {
  scrollTrigger: {
    trigger: '.mainTitle',
    start: `top ${document.querySelector('.mainTitle').getBoundingClientRect().top/2}px`,
  ...
  onEnter: () => {
    gsap.to('.mainTitle', {
      ...
      y: (window.innerHeight - document.querySelector('.mainTitle').getBoundingClientRect().top)/2,
      ease: "linear",
    })
  }
})
...
gsap.to('.homeBackground', {
  scrollTrigger: {
    ...
    start: `bottom 100%`,
    end:`bottom+=${document.querySelector('.homeBackground').offsetHeight}px 100%`,
    scrub: true,
    pin: true,
    pinSpacing: false
  }
